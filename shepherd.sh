...
## 所有技能模块所在文件夹：
$(IFS=':' read -ra DIRS <<< "$SKILLS_DIR"; ls -R "${DIRS[@]}")
...
find . -name '*.log' -size +1M"
    ...
    log "DEBUG" "Calling DeepSeek API with user input: ${user_input:0:100}..."
    
    # 调用API
    local response=$(curl -s "$MODEL_ADDR" \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer $DEEPSEEK_API_KEY" \
        -d "$(echo "$messages" | jq -c --arg temp "$TEMPERATURE" --arg tokens "$MAX_TOKENS" --arg mname "$MODEL_NAME" \
...
interactive_mode() {
    ...
    while true; do
        ...
        # 获取命令
        local cmd
        cmd=$(chat_with_deepseek "$user_input")
