---
name: shepherd
description: 智能 Shell 助手 (AI-powered CLI Agent), 是一个完全用 bash shell 写的专门生成 bash shell 脚本操作计算机的 AI Agent。代码极少, 依赖极少, 命令黑名单白名单，有心跳，可同时多agent，完全可控
---
...
## 本人的用法
手机ssh登陆服务器，用手机语音输入法输入（手机需要安装ssh客户端或者termux）。能做qq机器人或者AI小智交互吗？
...
# 心跳机制，通过指定心跳执行命令设置，例如
export shepherd_HEARTBEAT_CMD="${HOME}/.shepherd/heartbeat"  # 普通心跳执行命令，如监控，定时等常规任务
export shepherd_HEARTBEAT_S_CMD="${HOME}/.shepherd/heartbeat_s" # 命令前 source 了，最好只用于配置内部参数

# 注意心跳命令每 HEARTBEAT_TIME 秒执行一次，确保自己的命令不会卡死！

# 通过心跳执行命令可以动态改变参数，如动态加载技能在命令文件 shepherd_HEARTBEAT_S_CMD 里(可通过会话让shepherd生成命令改动)
SKILLS_DIR="新技能路径"

# 通过心跳执行命令可以实现技能，历史等的自动整理。skills里还有待开发心跳的技能。
